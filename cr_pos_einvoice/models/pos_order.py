...
class PosOrder(models.Model):
    ...
    def create_from_ui(self, orders, draft=False):
        # Capture existing order totals before sync to detect tip additions.
        existing_totals = {}
        for order_data in (orders or []):
            data = order_data.get("data", order_data) if isinstance(order_data, dict) else {}
            server_id = data.get("server_id") or data.get("id")
            if server_id:
                existing_order = self.browse(server_id).exists()
                if existing_order:
                    existing_totals[server_id] = existing_order.amount_total
        ...
        # If order totals changed (tip/service charge added), invalidate stale FE XML.
        for order in records:
            old_total = existing_totals.get(order.id)
            if old_total is not None and abs(order.amount_total - old_total) > 0.001:
                order._cr_invalidate_stale_fe_xml()
    ...
    def _cr_process_after_payment(self):
        for order in self:
            try:
                order._cr_dispatch_einvoice_flow()
            except Exception:  # noqa: BLE001
                # Never block the POS flow (tips, payments) due to FE processing errors.
                self._logger.exception("FE processing failed for POS order %s; will retry via cron.", order.name)
        ...
        for order in self:
            ...
            try:
                ...
                self._logger.exception(
                    "Unexpected error preparing TE for POS order %s: %s", order.name, error
                )
                order.write(
                    {
                        "cr_fe_status": "error_retry",
                        "cr_fe_error_code": "unexpected",
                        "cr_fe_last_error": str(error),
                        "cr_fe_next_try": fields.Datetime.now() + timedelta(minutes=5),
                    }
                )

    def _cr_invalidate_stale_fe_xml(self):
        """Remove existing FE XML when order lines changed (e.g. tip added after payment).

        This ensures the next FE processing run regenerates the XML with the
        updated totals including the tip/service charge.
        """
        for order in self:
            if order.cr_fe_status in self._CR_FINAL_STATES:
                continue
            if order.cr_fe_xml_attachment_id:
                order.write({
                    "cr_fe_xml_attachment_id": False,
                    "cr_fe_status": "draft" if order.cr_fe_status == "pending" else order.cr_fe_status,
                })
