...
class PosOrder(models.Model):
    ...
    cr_fe_invoice_requested = fields.Boolean(
        string="Solicitar FE (desde POS)",
        default=False,
        copy=False,
        tracking=True,
        help=(
            "Se marca cuando el operador activa 'Facturar' en el POS y la configuración "
            "está en modo FE sin factura contable (sin account.move)."
        ),
    )
    fp_invoice_template_style = fields.Selection(
        related="company_id.fp_invoice_template_style",
        string="Estilo plantilla FE",
        readonly=True,
    )
    ...
    @api.depends("account_move", "state", "amount_total", "lines.refunded_orderline_id", "cr_fe_invoice_requested")
    ...
    def _cr_is_marked_for_invoicing(self):
        """Return True when FE must be handled by a real account.move, not by POS-emitted documents.

        - Standard POS (config disabled): `to_invoice` / `invoice_status` define invoice-managed flow.
        - FE-from-order mode: only a persisted `account_move` makes the order invoice-managed.
        """
        self.ensure_one()
        config = self.config_id if self.config_id else self.env["pos.config"]
        if config and not getattr(config, "cr_fe_to_invoice_emits_fe", True):
            if "to_invoice" in self._fields and bool(self.to_invoice):
                return True
    ...
    def _cr_get_pos_document_type(self):
        ...
        if self._cr_is_refund_order_candidate():
            return "nc"
        if self.cr_fe_invoice_requested:
            return "fe"
        return "te"
    ...
    def _cr_get_pdf_report_action(self):
        ...
        # 1) Prefer POS-native reports so customer receives the same ticket style printed by POS/backoffice.
        ...
        for report in reports:
            if getattr(report, "report_name", "") == "cr_pos_einvoice.report_pos_order_fe_invoice" and doc_type != "fe":
                continue
            return report
    ...
    def _cr_get_or_create_pdf_attachment(self):
        ...
        doc_type = (self.cr_fe_document_type or self._cr_get_pos_document_type() or "").lower()
        raw_identifier = (self.cr_fe_consecutivo or self.name or (move and move.name) or f"POS-{self.id}").replace("/", "-")
        if doc_type == "fe":
            filename = f"FE_{raw_identifier}.pdf"
        elif doc_type == "nc":
            filename = f"NC_{raw_identifier}.pdf"
        else:
            filename = f"{raw_identifier}.pdf"
        if (
    ...
    def _cr_should_send_accepted_email(self):
        self.ensure_one()
        if not self._cr_is_auto_email_enabled():
            return False
        if self.cr_fe_status != "accepted" or self.cr_fe_email_sent:
            return False
        if not self._cr_get_customer_email():
            return False
        if self.cr_fe_document_type in ("te", "nc"):
            return True
        if self.cr_fe_document_type == "fe" and bool(getattr(self.config_id, "cr_fe_auto_email_include_fe", False)):
            return True
        return False
        return bool(
...
def _order_fields(self, ui_order):
    ...
    if auto_reference or manual_reference:
        ...
        if (
            merged_reference.get("cr_fe_reference_document_type")
            and merged_reference.get("cr_fe_reference_document_number")
            and merged_reference.get("cr_fe_reference_issue_date")
        ):
    ...
    # POS: use 'Facturar' only as FE document selector (no account.move) when enabled in config.
    payload = ui_order.get("data", ui_order) if isinstance(ui_order, dict) else {}
    to_invoice_ui = bool(payload.get("to_invoice")) if isinstance(payload, dict) else False
    to_invoice_vals = bool(fields_vals.get("to_invoice"))
    if to_invoice_ui or to_invoice_vals:
        config = (
            self.env["pos.config"].browse(fields_vals.get("config_id")).exists()
            if fields_vals.get("config_id")
            else self.env["pos.config"]
        )
        if config and getattr(config, "cr_fe_to_invoice_emits_fe", False):
            fields_vals["cr_fe_invoice_requested"] = True
            if "to_invoice" in fields_vals:
                fields_vals["to_invoice"] = False
...
def cr_get_virtual_move_for_fe_report(self):
    """Return an in-memory account.move for rendering the FE PDF from pos.order."""
    self.ensure_one()
    doc_type = (self.cr_fe_document_type or self._cr_get_pos_document_type() or "fe").upper()
    consecutivo = self.cr_fe_consecutivo or ""
    clave = self.cr_fe_clave or ""
    move = self._cr_build_virtual_move(document_type=doc_type, consecutivo=consecutivo, clave=clave)
    if not getattr(move, "name", False):
        move.name = consecutivo or self.name or self.pos_reference or f"POS-{self.id}"
    return move
