...
try:  # Prefer the canonical addon path when available.
    ...
    try:  # Allow local Python package imports (tests/dev without full addon registry).
        from l10n_cr_einvoice.services.einvoice_service import EInvoiceService as BaseEInvoiceService
    except ImportError:

        class BaseEInvoiceService:
            """Self-contained minimal FE service for POS fallback flows."""
            ...
            def _build_company_payload(self, company):
                return {
                    ...
                    "address": self._build_partner_payload(company.partner_id),
                }
